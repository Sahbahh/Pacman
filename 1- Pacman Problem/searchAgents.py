        ...
        for action in [Directions.NORTH, Directions.SOUTH, Directions.EAST, Directions.WEST]:
            ...
            if not self.walls[nextx][nexty]:
                ...
                if nextPosition in unvisitedCorners:
                    nextUnvisitedCorners = list(unvisitedCorners)
                    nextUnvisitedCorners.remove(nextPosition)
                    nextState = (nextPosition, tuple(nextUnvisitedCorners))
                else:
...
def foodHeuristic(state, problem):
    ...
    if not food_list:
        return 0
    distances = [manhattanDistance(position, food) for food in food_list]
    closest_food = min(distances)
    farthest_food = max(distances)

    return closest_food + (farthest_food - closest_food) // 2
